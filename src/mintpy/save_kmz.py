...
def plot_colorbar(
    ...
    figsize=(0.18, 3.6),
    nbins=None,
    label="Mean LOS velocity",
):
    fig, cax = plt.subplots(figsize=figsize)
    norm = colors.Normalize(vmin=vmin, vmax=vmax)
    cbar = colorbar.ColorbarBase(
        cax, cmap=plt.get_cmap(cmap), norm=norm, orientation="vertical"
    )
...
def generate_cbar_element(
    ...
    cbar_overlay = KML.ScreenOverlay(
        KML.name("colorbar"),
        KML.Icon(KML.href(f"{os.path.basename(cbar_file)}"), KML.viewBoundScale(0.75)),
...
def write_kmz_file(
    out_file_base, kml_doc, data_files=None, res_files=None, keep_kml_file=False
):
...
def save_kmz(inps):
    ...
    # mask
    mask = pp.read_mask(
        inps.file, mask_file=inps.mask_file, datasetName=inps.dset, box=inps.pix_box
    )[0]
    if mask is not None:
    ...
    data, inps.disp_unit, inps.disp_scale, inps.wrap = (
        pp.scale_data4disp_unit_and_rewrap(
            data,
            metadata=atr,
    ...
    if "Y_FIRST" in atr.keys():
        ...
        write_kmz_overlay(data, meta=atr, out_file=inps.outfile, inps=inps)

    else:
        # create placemark KML for file in radar-coord
        write_kmz_placemark(
            data, meta=atr, out_file=inps.outfile, geom_file=inps.geom_file, inps=inps
        )
