...
############# Global Variables ################
inps.tims, inps, img, mask, d_v, d_ts = None, None, None, None, None, None
ax_v, fig_ts, fig_v, ax_ts, tslider, second_plot_axis, new_axes = None, None, None, None, None, None, None
h5, k, dateList, atr, inps.num_date = None, None, None, None, None
...
def read_error_list():
    ...
    if inps.error_file:
        ...
        if inps.ex_date_list:
            ...
            inps.error_ts = np.array([e_ts[i] for i in range(inps.num_date) if i not in inps.ex_idx_list])
...
def set_zero_displacement():
    ...
    if inps.zero_first:
        if inps.ex_date_list:
            inps.zero_idx = min(list(set(range(inps.num_date)) - set(inps.ex_idx_list)))
        else:
            inps.zero_idx = 0
...
def compute_file_size():
    ...
    print(('data size in [y0,y1,x0,x1]: [%d, %d, %d, %d]' % (0, inps.length, 0, inps.width)))


def compute_lat_lon_params():
    """Computes parameters needed for setting lat/lon values"""
    global inps, ullon, ullat, lon_step, lat_step, atr
    ...
    try:
        ...
        lon_step = float(atr['X_STEP'])
        lat_step = float(atr['Y_STEP'])
        lrlon = ullon + inps.width * lon_step
        lrlat = ullat + inps.length * lat_step
...
def make_time_slider():
    global tslider, fig_v, inps

    ax_time = fig_v.add_axes([0.03, 0.10, 0.30, 0.07], facecolor='lightgoldenrodyellow', yticks=[])
    tslider = Slider(ax_time, '', inps.tims[0], inps.tims[-1], valinit=inps.tims[inps.epoch_num])
    tslider.ax.bar(inps.tims, np.ones(len(inps.tims)), facecolor='black', width=0.01, ecolor=None)

    # xaxis tick format
    if np.floor(inps.tims[-1]) == np.floor(inps.tims[0]):
        digit = 10.
    else:
        digit = 1.
    tslider.ax.set_xticks(np.round(np.linspace(inps.tims[0], inps.tims[-1], num=5) * digit) / digit)
...
def format_coord(x, y):
    ...
    global inps, ullat, lat_step, ullon, lon_step, d_v, lat, lon
...
def plot_timeseries_errorbar(ax, dis_ts, inps):
    ...
    if inps.ex_date_list:
        ...
        d_ts = np.array([dis_ts[i] for i in range(inps.num_date) if i not in inps.ex_idx_list])
        # Plot excluded dates
...
def estimate_slope():
    ...
    global inps, inps.tims, d_ts, inps.num_date

    if inps.ex_date_list:
        tims_kept = [inps.tims[i] for i in range(inps.num_date) if i not in inps.ex_idx_list]
        d_ts_kept = [d_ts[i] for i in range(inps.num_date) if i not in inps.ex_idx_list]
        d_slope = stats.linregress(np.array(tims_kept), np.array(d_ts_kept))
    else:
        d_slope = stats.linregress(np.array(inps.tims), np.array(d_ts))
...
def update_annot(ind, sc):
    global p1_x, p1_y, p2_x, p2_y, annot, p1_scatter, p2_scatter, inps.tims, lat, lon
