...
def update_inps_with_file_metadata(inps, metadata):
    ...
    # Colormap
    inps.colormap = pp.check_colormap_input(metadata, inps.colormap, datasetName=inps.dset[0])
    ...
    if inps.seed_yx and inps.geo_box:
        ...
        inps.seed_lalo = [float(metadata['REF_LAT']), float(metadata['REF_LON'])]
    else:
        inps.seed_lalo = None

    # Unit and Wrap
    inps.disp_unit, inps.wrap = pp.check_disp_unit_and_wrap(metadata, disp_unit=inps.disp_unit, wrap=inps.wrap)
    ...
    if not inps.fig_title:
        inps.fig_title = pp.auto_figure_title(metadata['FILE_PATH'], datasetNames=inps.dset, inps_dict=vars(inps))
...
def plot_2d_matrix(ax, data, metadata, inps=None):
    ...
    if inps.dem_file:
        ...
        dem, dem_metadata = readfile.read(inps.dem_file, datasetName='height', box=inps.dem_pix_box)
    ...
    if inps.geo_box and inps.fig_coord=='geo':
        ...
        if inps.dem_file:
            ...
            m = pp.plot_dem_background(ax=m, geo_box=inps.geo_box, dem=dem, inps_dict=vars(inps))
        ...
        if inps.disp_scalebar:
            ...
            if inps.scalebar[0] == 999.0:
                ...
                wid_dist = gc.inv(inps.geo_box[0], inps.geo_box[3], inps.geo_box[2], inps.geo_box[3])[2]
    ...
    #-------------------- 2.2 Plot in Y/X-coordinate ------------------------------------------------#
        ...
        if inps.dem_file:
            print('plotting DEM background ...')
            ax = pp.plot_dem_background(ax=ax, geo_box=None, dem=dem, inps_dict=vars(inps))
...
def read_data4figure(inps, i_start, i_end):
    ...
    for i in range(i_start, i_end):
        d = readfile.read(inps.file, datasetName=inps.dset[i], box=inps.pix_box, print_msg=False)[0]
