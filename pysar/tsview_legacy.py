...
def read_timeseries_yx(timeseries_file, y, x, ref_yx=None):
    ...
    if k in ['GIANT_TS']:
        ...
        date_list = [dt.fromordinal(int(i)).strftime('%Y%m%d')
...
def main(iargs=None):
    ...
    if inps.ex_date_list:
        ...
        if input_ex_date:
            for ex_date in input_ex_date:
                ...
                inps.ex_date_list += list(set(ex_date) -
                                          set(inps.ex_date_list))
            ...
            inps.ex_idx_list = sorted([dateList.index(i)
    ...
    img = ax_v.imshow(d_v, cmap=inps.colormap,
                      clim=inps.ylim_mat, interpolation='nearest')
    ...
    tslider.ax.set_xticks(np.round(np.linspace(
        inps.yearList[0], inps.yearList[-1], num=5)*100)/100)
    ...
    inps.error_ts = None
    if inps.error_file:
        error_fileContent = np.loadtxt(
            inps.error_file, dtype=bytes).astype(str)
    ...
    def plot_timeseries_errorbar(ax, dis_ts, inps):
        ...
        if inps.ex_date_list:
            ...
            d_ts = np.array([dis_ts[i] for i in range(
                date_num) if i not in inps.ex_idx_list])
            # Plot excluded dates
    ...
    def plot_timeseries_scatter(ax, dis_ts, inps):
        ...
        if inps.ex_date_list:
            ...
            d_ts = np.array([dis_ts[i] for i in range(
                date_num) if i not in inps.ex_idx_list])
            # Plot excluded dates
            ax.scatter(inps.ex_dates, ex_d_ts, s=inps.marker_size **
                       2, color='gray')   # color='crimson'
        # Plot kept dates
    ...
    def update_timeseries(ax_ts, y, x):
        ...
        ax_ts = pp.auto_adjust_xaxis_date(
            ax_ts, inps.yearList, fontSize=inps.font_size)[0]
        ax_ts.set_xlabel('Time', fontsize=inps.font_size)
        ax_ts.set_ylabel('Displacement [%s]' %
                         inps.disp_unit, fontsize=inps.font_size)
        ...
        # Slope estimation
        if inps.ex_date_list:
            inps.yearList_kept = [inps.yearList[i]
                                  for i in range(date_num) if i not in inps.ex_idx_list]
            d_ts_kept = [d_ts[i]
                         for i in range(date_num) if i not in inps.ex_idx_list]
            d_slope = stats.linregress(
                np.array(inps.yearList_kept), np.array(d_ts_kept))
        else:
    ...
    if inps.save_fig and inps.yx:
        ...
        try:
            ...
            pass
        if inps.ref_yx:
            header_info += '\nreference pixel: y=%d, x=%d' % (
                inps.ref_yx[0], inps.ref_yx[1])
        else:
            header_info += '\nreference pixel: y=%s, x=%s' % (
                atr['REF_Y'], atr['REF_X'])
        header_info += '\nunit=m/yr'
        ...
        fig_ts.savefig(outName, bbox_inches='tight',
        ...
        fig_v.savefig(outName, bbox_inches='tight',
                      transparent=True, dpi=inps.fig_dpi)
