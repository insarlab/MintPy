...
def read_fault_coords(Fault_coord_file,Dp):
  ...
  for i in range(Np-1):
   ...
   # Fault_lat.append(Coords[i,1])
    Xf0=Coords[i,0];Yf0=Coords[i,1]
    Xf1=Coords[i+1,0];Yf1=Coords[i+1,1]
    DY=Yf1-Yf0;DX=Xf1-Xf0
    ...
    if Fault_Segment<=Dp:
       ...
       while Fault_Segment>L:
          ii=ii+1
          x,y=point_on_line_with_distance_from_beginning(Xf0,Yf0,Xf1,Yf1,Dp*ii)
          Dx=y-Yf0;Dy=x-Xf0
          L = np.hypot(Dx, Dy)
          if L<=Fault_Segment:
            Fault_lon.append(x)
