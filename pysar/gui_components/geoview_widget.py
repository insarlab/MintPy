...
class GeoViewWidget(qt.QWidget):

    def __init__(self, parent=None, iargs=None):

        super(GeoViewWidget, self).__init__(parent)
        ...
        self.fig, self.canvas, self.toolbar = self.main(iargs)
    ...
    def main(self, args=None):
        ...
        if not self.inps.disp_fig:
            plt.switch_backend('Agg')  # Backend setting

        self.inps, atr = self.check_input_file_info(self.inps)
###
        if self.inps.disp_setting_file:
            self.inps = self.update_inps_with_display_setting_file(self.inps, self.inps.disp_setting_file)

        self.inps = self.update_inps_with_file_metadata(self.inps, atr)
        self.inps = self.read_mask(self.inps, atr)

        # Create new figure object
        self.fig = Figure(self.inps.fig_size)
        ...
        ############################### One Subplot ###############################
        if self.inps.dsetNum == 1:
            print('reading data ...')
            data, atr = readfile.read(self.inps.file,
                                      datasetName=self.inps.dset[0],
                                      box=self.inps.pix_box,
                                      print_msg=False)
            if self.inps.ref_date:
                data -= readfile.read(self.inps.file,
                                      datasetName=self.inps.ref_date,
                                      box=self.inps.pix_box,
                                      print_msg=False)[0]
            if self.inps.zero_mask:
                data[data == 0] = np.nan
        ...
        ############################### Multiple Subplots #########################
            ...
            self.inps = self.prepare4multi_subplots(self.inps)

            self.inps.all_data_min = 0
            self.inps.all_data_max = 0
            for j in range(1, self.inps.fig_num + 1):
                self.plot_figure(self.inps, j)

            print('----------------------------------------')
            print('all data range: [%f, %f] %s' % (self.inps.all_data_min, self.inps.all_data_max, self.inps.disp_unit))
            if self.inps.disp_min and self.inps.disp_max:
                print('display  range: [%f, %f] %s' % (self.inps.disp_min, self.inps.disp_max, self.inps.disp_unit))
    ...
    def check_input_file_info(self, inps):
        # File Baic Info
        if not os.path.isfile(self.inps.file):
        ...
        inps, atr = self.read_dataset_input(inps)

        return inps, atr

    def read_dataset_input(self, inps, print_msg=True):
        ...
        if len(inps.dset) > 0 or len(inps.dsetNumList) > 0:
            inps.dsetNumList = self.check_dataset_input(inps.fileDatasetList,
                                                   inps.dset,
                                                   inps.dsetNumList,
                                                   inps.globSearch)[1]
        ...
        inps.exDsetList, inps.exDsetNumList = self.check_dataset_input(inps.fileDatasetList,
                                                                  inps.exDsetList,
                                                                  [],
                                                                  inps.globSearch)
        ...
        if inps.ref_date:
            if inps.key not in timeseriesKeyNames:
                inps.ref_date = None
            ref_date = self.check_dataset_input(inps.fileDatasetList,
                                           [inps.ref_date],
                                           inps.globSearch)[0][0]
            if not ref_date:
                if print_msg:
    ...
    def check_dataset_input(self, allList, inList=[], inNumList=[], globSearch=True):
    ...
    def update_inps_with_display_setting_file(self, inps, disp_set_file):
    ...
    def update_inps_with_file_metadata(self, inps, metadata):
        # default mask file:
        ...
        inps = self.update_figure_setting(inps)
        return inps

    def update_figure_setting(self, inps):
    ...
    def update_data_with_plot_inps(self, data, metadata, inps):
        # Seed Point
    ...
    def read_mask(self, inps, atr):
        ...
        if os.path.isfile(str(inps.mask_file)):
            try:
                ...
                print('Can not open mask file: ' + str(inps.mask_file))
                inps.mask_file = None
    ...
    def plot_2d_matrix(self, ax, data, metadata, inps=None):
        """Plot 2D matrix
        ...
        if not inps:
            inps = self.cmd_line_parse([''])
            inps = self.update_inps_with_file_metadata(inps, metadata)

        # ----------------------- 1. Update plot inps/data with data matrix -----------------------#
        data, inps = self.update_data_with_plot_inps(data, metadata, inps)
        # Multilook
    ...
    def prepare4multi_subplots(self, inps):
        ...
        if inps.multilook and inps.multilook_num == 1:
            ...
            if auto_multilook:
                inps.multilook, inps.multilook_num = self.check_multilook_input(inps.pix_box,
                                                                           inps.fig_row_num,
                                                                           inps.fig_col_num)
    ...
    def check_multilook_input(self, pixel_box, row_num, col_num):
    ...
    def plot_figure(self, inps, j):
        ...
        data = self.read_data4figure(inps, i_start, i_end)

        # Loop - Subplots
        prog_bar = ptime.progressBar(maxValue=i_end - i_start, prefix='ploting')
        for i in range(i_start, i_end):
            ax = self.fig.add_subplot(inps.fig_row_num, inps.fig_col_num, i - i_start + 1)
            im = self.plot_subplot4figure(inps, ax=ax, data=data[i - i_start, :, :], i=i)
        ...
        if not inps.disp_min and not inps.disp_max:
            ...
            print('show colorbar')
            self.fig.subplots_adjust(right=0.93)
            cax = self.fig.add_axes([0.94, 0.3, 0.005, 0.4])
            inps, cax = pp.plot_colorbar(inps, im, cax)

        # Save Figure
        if inps.save_fig:
            print('save figure to ' + inps.outfile)
            self.fig.savefig(inps.outfile, bbox_inches='tight',
                        transparent=True, dpi=inps.fig_dpi)
            if not inps.disp_fig:
                self.fig.clf()
        return

    def read_data4figure(self, inps, i_start, i_end):
    ...
    def plot_subplot4figure(self, inps, ax, data, i):
    ...
    def cmd_line_parse(self, iargs=None):
        ...
        self.inps = parser.parse_args(args=iargs)

        # If output flie name assigned or figure shown is turned off, turn on the figure save
        if self.inps.outfile or not self.inps.disp_fig:
            self.inps.save_fig = True
        if self.inps.coastline and self.inps.resolution in ['c', 'l']:
            self.inps.resolution = 'i'
        if self.inps.lalo_step:
            self.inps.lalo_label = True
        return self.inps
...
if __name__ == '__main__':
    ...
    main = GeoViewWidget(iargs=sys.argv[1:])
    main.show()
