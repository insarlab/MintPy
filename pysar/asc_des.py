...
#   heading2=np.pi*193.4/180.
   ...
   try:
      ...
      theta1=float(h5V1['velocity'].attrs['LOOK_REF2'])
   print 'Look angle 1: '+str(theta1)

   try:
      theta2=float(sys.argv[5])
   except: 
      theta2=float(h5V2['velocity'].attrs['LOOK_REF2'])
...
#   A[0,0]=np.cos(theta1);A[0,1]=-np.sin(theta1);
#   A[1,0]=np.cos(theta2);A[1,1]=-np.sin(theta2);
   A1=np.linalg.pinv(A)
   Luh=np.dot(A1,data)
