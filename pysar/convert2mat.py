...
#  print h5file['velocity'].attrs['WAVELENGTH']
  k=h5file.keys()
...
#    velocity['rate']=data
    ...
    try: 
      velocity['wavelength']=float(h5file[k[0]].attrs['WAVELENGTH'])
    except:
      print 'WAVELENGTH was not found'
    try:
      velocity['sat_height']=float(h5file[k[0]].attrs['HEIGHT'])
    except:
      print 'HEIGHT was not found'

    try:
      velocity['near_range']=float(h5file[k[0]].attrs['STARTING_RANGE'])
    except:
    ...
    try:
      velocity['near_LookAng']=float(h5file[k[0]].attrs['LOOK_REF1'])
    except:
      print 'LOOK_REF1 was not found'
    try:
      velocity['far_LookAng']=float(h5file[k[0]].attrs['LOOK_REF2'])
    except:
    ...
    velocity['sat']=''
    try:
      velocity['width']=int(h5file[k[0]].attrs['WIDTH'])
    except:
      print 'WIDTH was not found'

    try:
      velocity['file_length']=int(h5file[k[0]].attrs['FILE_LENGTH'])
    except:
      print 'FILE_LENGTH was not found'
    velocity['t']=''
    velocity['date']=''
    velocity['date_years']=''
    try:
       velocity['sat']   = h5file[k[0]].attrs['satellite']
    except:
       velocity['sat']   = ''
