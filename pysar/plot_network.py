...
def create_parser():
    ...
    coh.add_argument('-t','--template', dest='template_file', help='template file with options below:\n'+TEMPLATE)
    ...
    coh.add_argument('--mask', dest='maskFile', default='mask.h5', help='mask file used to calculate the coherence')
    ...
    fig.add_argument('--fs','--fontsize', type=int, default=12, help='font size in points')
...
def main(iargs=None):
    ...
    if inps.cohList is not None:
        ...
        fig, ax = plt.subplots(figsize=inps.fig_size)
        ax = pp.plot_coherence_matrix(ax, inps.date12List, inps.cohList, inps.date12List_drop, plot_dict=vars(inps))
        if inps.save_fig:
        ...
        fig, ax = plt.subplots(figsize=inps.fig_size)
        ax = pp.plot_coherence_history(ax, inps.date12List, inps.cohList, plot_dict=vars(inps))
        if inps.save_fig:
    ...
    fig, ax = plt.subplots(figsize=inps.fig_size)
    ax = pp.plot_network(ax, inps.date12List, inps.dateList, inps.pbaseList, vars(inps), inps.date12List_drop)
    if inps.save_fig:
