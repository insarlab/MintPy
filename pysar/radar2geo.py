...
from pysar.objects import ifgramStack, timeseries, geometry, resample
...
pysar.geocode.latStep      = auto  #[0.0-90.0 / None], auto for None - calculate from lookup file, output resolution in degree
pysar.geocode.lonStep      = auto  #[0.0-180.0 / None], auto for None - calculate from lookup file, output resolution in degree
...
def createParser():
    parser = argparse.ArgumentParser(description='Resample File in radar coordinates into geo coordinates',\
                                     formatter_class=argparse.RawTextHelpFormatter,\
                                     epilog=TEMPLATE+'\n'+EXAMPLE)

    parser.add_argument('file', nargs='+', help='File(s) to be geocoded')
    parser.add_argument('-l','--lookup', dest='lookupFile', help='Lookup table file generated by InSAR processors.')
    parser.add_argument('-t','--template', dest='templateFile', help='Template file with geocoding options.')
    ...
    parser.add_argument('-i','--interpolate', dest='interpMethod', default='nearest', choices={'nearest','bilinear'},\
    ...
    parser.add_argument('-o','--output', dest='outfile', nargs='*', help="output file name. Default: add prefix 'geo_'")
...
def read_template2inps(templateFile, inps):
    ...
    for key in keyList:
        ...
        if value:
            if key == 'SNWE':
                inpsDict[key] = tuple([float(i) for i in value.split(',')])
...
############################################################################################
def update_attribute(atrIn, inps, printMsg=True):
    atr = dict(atrIn)
    ...
    if 'REF_Y' in atr.keys():
        ref_lat, ref_lon = ut.radar2glob(np.array(int(atr['REF_Y'])), np.array(int(atr['REF_X'])),\
                                         inps.lookupFile, atrIn, printMsg=False)[0:2]
...
def geocode_file(infile, inps, resObj, outfile=None):
    print('-'*50)
    ...
    geo_data = resObj.resample(data, inps.src_def, inps.dest_def, inps.interpMethod, inps.fillValue)
...
def main(iargs=None):
    ...
    resObj = resample(lookupFile=inps.lookupFile, dataFile=inps.file[0], SNWE=inps.SNWE, laloStep=inps.latStep)
    inps.src_def, inps.dest_def, inps.SNWE = resObj.get_geometry_definition()

    for infile in inps.file:
        geocode_file(infile, inps, resObj)
