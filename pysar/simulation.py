...
def usage():
    print '''
****************************************************************************************
  Simulating a set of interferograms based on real interferograms and an existing 
      displacement velocity field.

  simulation.py -i interferograms -v velocity -u unwrapping_error_percentage
                -m mask for unwrapping error addition -y susbet in y -x subste in x -o output name
...
def main(argv):
    ...
    try:      opts, args = getopt.getopt(argv,"h:i:v:u:m:o:x:y:")
    except getopt.GetoptError:   usage() ; sys.exit(1)
  
    for opt,arg in opts:
        if opt in ("-h","--help"):        usage();        sys.exit()
        elif opt == '-i':        igramFile = arg
    ...
    #####################################################
    try:     ysub
    except:  ysub=[0,mset.shape[0]]
    
    try:     xsub
    except:  xsub=[0,mset.shape[1]]  
    #####################################################
